...
class EventPlaylistsController(CustomController):
    ...
    def post(self, request: Request, eventId: int) -> Response:
        def actionCall(**kwargs):
            return Event(id=kwargs.get("id")).linkToPlaylist(
                kwargs["data"]["playlist"]["id"]
            )

        return self.link(request=request, actionCall=actionCall, objectId=eventId, serializer=EventPlaylistsSerializer)
